  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        await resp.json();
        onSave?.(taskData); // Changed from onSave?.() to onSave?.(taskData)
        onClose();
      } catch (error) {
        console.error(error);
      ...
      <div className='bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div>
            ...
            <div className='flex items-center border-purple-100 rounded-lg px-3 py-2.5 focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500 transition-all duration-200'>
              <input
                type='text'
                name='title'
                required
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div>
              <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                <Flag className='w-4 h-5 text-purple-500' /> Priority
              </label>
              <select
                name='priority'
            ...
            <div>
              <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                <Calendar className='w-4 h-5 text-purple-500' /> Due Date
              </label>
              <input
                type='date'
                name='dueDate'
          ...
          <div>
            <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-2'>
              <CheckCircle className='w-4 h-5 text-purple-500' /> Status
            </label>
