...
import {BUTTONCLASSES, Inputwrapper,MESSAGE_SUCCESS} from '../assets/dummy'
import {MESSAGE_ERROR, FIELDS} from '../assets/dummy'
...
const SignUp = ({onSwitchMode}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({text: error.respomse?.data?.message || "Signup failed", type:"error"});
    }
    finally{
      setLoading(false);
    }
    ...
    <div className='max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8'>
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <button type='submit' className={BUTTONCLASSES} disabled={loading}>
          {loading ? "Signing Up..." : <><UserPlus className='w-h h-4'/>Sign Up</>}
        </button>
      </form>
