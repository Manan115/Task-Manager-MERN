...
import { ChevronDown, LogOut, Zap, Settings } from "lucide-react";
      ...
      <div className="flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto">
        ...
        <div className="flex item-center gap-4">
          ...
          <div ref={menuref} className="relative">
            ...
            >
              ...
              <div className="relative">
                {user.avatar ? (
                  ...
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-fuchsia-500 to-purple-600 text-white font-semibold flex items-center justify-center shadow-md">
                    {user.name?.[0]?.toUpperCase() || "U"}
                  </div>
                )}

                {/* Green Pulse Dot */}
                <div className="absolute w-3 h-3 -bottom-0.5 -right-0.5 bg-green-400 rounded-full border-2 border-white animate-pulse" />
              </div>
            ...
            {/* Dropdown Menu */}
            {menuOpen && (
                <ul className='absolute top-14 right-0 w-56 bg-white rounded-2xl shadown-xl border border-purple-100 z-50 overflow-hidden animate-fadeIn'>
                       <li className='p-2'>
                            <button onClick={()=>{
                                    setMenuOpen(false);
                                    navigate('/profile');
                                }}
                            className='w-full px-4 py-2.5 text-left text-gray-700 hover:bg-purple-50 transition-colors text-sm flex items-center gap-2 group rounded-lg' role='menuitem'>
