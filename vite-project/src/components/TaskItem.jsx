...
const TaskItem = ({task,onRefresh,onLogout,showCompleteCheckbox=true}) => {
    ...
    try {
        const taskId = task._id || task.id; // Use whichever ID exists
    ...
    try {
        ...
        const response = await axios.put(`${API_BASE}/${taskId}/gp`, payload, {
            headers: getAuthHeaders()
        });
        
        console.log('Task updated successfully:', response.data); // Debug log
        
        setShowEditModal(false);
        
        // Ensure refresh happens after state update
        if (onRefresh) {
            await onRefresh();
        }
    } catch(err) {
